...
import type1 from "./imgs/forest.jpg";
import type2 from "./imgs/fireplace.jpg";
import type3 from "./imgs/sea.jpg";
...
const RoundDivSpan = styled.span`
  ...
  text-align: center;
`;
export default ({ bgColor2, setBgColor4, Q4Answer, setQ4Answer }) => {
  //문항은 추후에 DB에서 추출
  const Q = "무드(분위기)를 골라주세요";
  const A = ["Calm Forest", "Cozy Home", "Cool Sea", "CozyHome"];
  ...
  return (
    <>
      ...
      <QSubSpan>
        A.I Customize #04 /<QSubGraySpan> 05</QSubGraySpan>
      </QSubSpan>
      <QSpan>Q4.{Q}</QSpan>
      <QDetailSpan>무드를 상상해봅시다</QDetailSpan>
      <QDetailSpan>선호하시는 분위기를 선택해주세요</QDetailSpan>
      <QSelectContainer>
        <QSelectList>
          <QSelectDrawer
            onClick={() => {
          ...
          >
            ...
            {click === A[0] ? (
              <DetailBox>
                <QRedSpan fontsize={"24px"}>Calm Forest</QRedSpan>
          ...
          >
            ...
            {click === A[1] ? (
              <DetailBox>
                <QRedSpan fontsize={"24px"}>Cozy Home</QRedSpan>
                <QDetailSpan>감귤류부터 과즙이 풍부한 과일까지</QDetailSpan>
                <QDetailSpan>신선하고 과일맛이 나는 티들이</QDetailSpan>
                <QDetailSpan>당신의 하루를 밝게 만들어줄거에요</QDetailSpan>
              </DetailBox>
          ...
          >
            ...
            {click === A[2] ? (
              <DetailBox>
                <QRedSpan fontsize={"24px"}>Cool Sea</QRedSpan>
                <QDetailSpan>감귤류부터 과즙이 풍부한 과일까지</QDetailSpan>
                <QDetailSpan>신선하고 과일맛이 나는 티들이</QDetailSpan>
                <QDetailSpan>당신의 하루를 밝게 만들어줄거에요</QDetailSpan>
              </DetailBox>
